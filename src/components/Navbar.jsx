    ...
    <>
      <header className={` z-50 sticky top-0 bg-white ${scrolled && 'bg-slate-100/70 backdrop-blur shadow'}  `}>
        <div className="z-10 relative">
          ...
          <nav
            className={` w-full md:hidden bg-accentGreen pt-28 pb-5 shadow transition-all duration-200 ease-in-out absolute z-0 ${
              open ? 'top-0  opacity-100' : '-top-full opacity-0'
            } h-fit`}
          >
            <ul className=" z-20 w-11/12 mx-auto flex  gap-4 text-main font-semibold flex-col ">
              {navlink.map((nav) => (
